...
String SOFTWARE_DATE = "2024.01.16";
...
void loop() {
  ...
  if (millis() - lastSecond >= 1000) {
    ...
    if (++TXcounter == TXPERIOD || FIRSTLOOP) {       // if TX period has been reached and at boot
      ...
      // read BOSCH BME sensor if exists
      #if defined(WITH_BME280) || defined(WITH_BME680)
        BME280_Utils::read();
      #endif
      ...
      // after all measures have been done, display on OLED
      //  Utils::PrintOLED();
    ...
    #ifdef DEBUG_TIME
        logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "TIME", "sec/2min/min/10min/TX");
        logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "TIME", "%d/%d/%d/%d/%d",seconds,seconds_2m,minutes,minutes_10m,TXcounter);
    #endif
  }  // end if millis > 1000
