...
//extern long microSecondsSinceBoot;
...
extern volatile int windDirAvg[120];             // array of 120 ints to keep track of 2 minute average
extern int windDir_avg2m;     // 2 minute average wind direction in degrees 0-360
...
extern volatile float windSpeedAvg[120];         // to keep track of last 120 wind speed measures
...
void PrintOLED() {
    ...
    // formatting line2
    #if defined(WITH_BME280) || defined(WITH_BME680) // if we have a pressure to display
    ...
    // formatting line6    
    #ifdef WITH_WIND
      line6 = "WIND: " + String(windSpeed_avg2m,1) + " km/h " + String(windDir_avg2m) + (char)247;
    #else
      line6="";
    #endif
...
*/
  ...
  #ifdef DEBUG_WIND
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "RS485", "currentSpeed km/h : %s km/h",String(currentSpeedKmh));
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "RS485", "windSpeed_avg2m : %s km/h",String(currentSpeedKmh));
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "RS485", "Gust speed : %s km/h",String(windgustSpeed));
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "RS485", "Gust dir : %s deg",String(windgustDir));
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "RS485", "CurrentDir : %s deg",String(currentDir));
    logger.log(logging::LoggerLevel::LOGGER_LEVEL_DEBUG, "RS485", "windDir_avg2m : %s deg",String(windDir_avg2m));
    //Serial.println();
  #endif
  }
