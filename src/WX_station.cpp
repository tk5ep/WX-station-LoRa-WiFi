...
Updates :
---------
121223 Added SHT31 sensor and reorder of the script. Some cosmetics.
...
| \__/\ (_) | | | | | | | (_| | |_| | | | (_| | |_| | (_) | | | |
 \____/\___/|_| |_|_| |_|\__, |\__,_|_|  \__,_|\__|_|\___/|_| |_|
                         ...
                         |___/                                   
********************************************************************/
String SOFTWARE_VERSION = "1.1" ;
String SOFTWARE_DATE = "12-12-23";
...
void setup() {
  ...
  // what was the last reset reason ?
...
void loop() {
  ...
  if (millis() - lastSecond >= 1000) {
    ...
    if (++TXcounter == TXPERIOD || FIRSTLOOP) {                      // if TX period has been reached and at boot
          FIRSTLOOP = false;                                               // we made at least one loop
          #ifdef WITH_BME280
            readBME();
          #endif
          #ifdef WITH_SHT31
...
#ifdef WITH_SHT31
void readSHT31() {
    sht.getError();                   // clear the error flag
    if (sht.getError() != 0) {        // read the error flag
